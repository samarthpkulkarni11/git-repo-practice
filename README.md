...
5. Reword, Drop, Squash and Edit Raised Commits

   ```bash
   git rebase -i HEAD~n -> n here is number of commits
   ```

6. To get more details with indexes, commit ID, and general info on commits made for a particular repo

   ```bash
   git reflog
   ```

7. To check changes between commits, commit and working tree

   ```bash
   git diff
   ```

8. To reset the branch to a particular commit or origin
